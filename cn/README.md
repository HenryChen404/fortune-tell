...
**规则二：多数一致才输出。** 设适用体系数为 N，只有 ≥ N-1 个体系结论一致的点才会输出。2 个体系适用，必须 2 个都同意；3 个适用，至少 2 个同意。不满足阈值的解盘直接丢弃，不会出现"A 体系认为…但 B 体系不支持"这种噪音。

**规则三：古话翻译成人话。** 这些体系发明于农业社会。"官星被克"不等于你要被贬官，而是你的权威感、管理能力、职场地位在这段时间承压。所有解盘自动映射到现代语境。
...
首次排盘后，系统会自动进入验盘阶段。命盘中的许多原局象（如七杀、太阳落12宫、Rahu-Ketu轴等）具有多义性——同一个符号在不同人身上走完全不同的方向。验盘通过回溯验证来解决这个问题：

1. **读取全部命盘**，识别跨体系共振原局象和单体系多义原局象
2. **动态生成问题**，每个问题绑定具体的大运/流年/Dasha时间段，提供2-4个可验证的选项
3. **收集回答**，支持"不确定"（标记为未验盘）和"都不符合"（触发追问探索新方向），每题可附自由文字补充
4. **保存验盘数据**到各体系独立的验盘档案（`bazi_calibration.md` 等），一个跨体系问题的结果会同时写入相关体系的文件

验盘不是一次性的——后续解盘中如果命主反馈不准，可以随时增量验盘。
...
2. **验盘后**：跨体系共振扫描——八字、西洋、吠陀三套命盘中有多少与命宫主星产生共鸣，决定进化等级（R → SR → SSR → SSSR）
...
└── references/            # (git-ignored) 生成的个人命盘数据
    └── <档案名>/           # 每个人一个独立档案
        ...
        ├── bazi_calibration.md   # 八字验盘数据
        ├── ziwei_calibration.md  # 紫微验盘数据
        ├── western_calibration.md # 西洋验盘数据
        └── vedic_calibration.md   # 吠陀验盘数据
```
